...
/**
 * Modified by Sergio Mira on 11/6/2014
 * Added option to work with remote server (needs libssh2 - http://php.net/manual/en/book.ssh2.php)
 * Modified some comments, include __construct doc comments
 */
...
class IptablesConfig
{
     ...
     * @param int $ipversion Iptables version
     * @param string $sudo_cmd To use sudo or not (If yes, construct with 'sudo' in this variable)
     * @param mixed $remote_server Conection resource from previous ssh2_connect(), if working with remote server
     * @return void
     */
    function __construct($rulesFile = NULL, $ipversion = 4, $sudo_cmd = '', $remote_server = 'n')
    {
        $this->debug = false;
        ...
        if ($rulesFile == NULL) {
            ...
            if (is_executable($this->ipt_save)) {
                if (is_string($this->remote_server) && $this->remote_server == 'n') {
                    exec($this->sudo_cmd . $this->ipt_save . ' -c', $output, $return_val);
                    if ($return_val == 0 && $this->remote_server == 'n') {
                ...
                } elseif (!empty($this->remote_server)) { /* Adapted to work with remote server */
                    $fileString = ssh2_exec($this->remote_server, $this->sudo_cmd . $this->ipt_save . ' -c');
                    stream_set_blocking($fileString, true);
                    $fileString = ssh2_fetch_stream($fileString, SSH2_STREAM_STDIO);
                    $fileString = stream_get_contents($fileString);
                }
            }
            else
    ...
    public function commit($file = NULL)
    {
        ...
        /* Write remotely the file, if working with remote server */
        if (!empty($this->remote_server)) {
            $saida = ssh2_scp_send($this->remote_server, $file, $file, 0644);
            if ($saida === false) {
                return false;
            }
        }
    ...
    public function applyNow($restoreCounters = true, $iptRestore = NULL, $fileName = NULL)
    {
        ...
        if (is_executable($iptRestore)) {
            ...
            $comando = $this->sudo_cmd . $iptRestore . ' ' . $tmp_path;

            if (is_string($this->remote_server) && $this->remote_server == 'n') {
                exec($comando, $output, $return_val);
                if ($return_val == 0) {
                    return true;
                } else {
                    $str = '';
            ...
            } elseif (!empty($this->remote_server)) {
                $saida = ssh2_exec($this->remote_server, $comando);
                if ($saida === false) {
                    return false;
                } else {
                    // Salva as configuracoes no arquivo oficial para ficar atualizado se/quando reiniciar o servidor
                    $saida = ssh2_exec($this->remote_server, "{$this->ipt_save} > /etc/sysconfig/iptables");
                    if ($saida === false) {
                        //exit($saida);
                        return false;
                    } else {
